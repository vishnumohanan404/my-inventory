...
func TestMain(m *testing.M) {
	err := a.Initialize(DbUser, "test")
	if err != nil {
		log.Fatal("Error occured while initialising the db")
	}
	createTable()
	m.Run()
}
...
func checkStatusCode(t *testing.T, expectedStatusCode int, actualStatusCode int) {
	if expectedStatusCode != actualStatusCode {
		t.Errorf("Expected status: %d, Received: %d", expectedStatusCode, actualStatusCode)
	}
}
